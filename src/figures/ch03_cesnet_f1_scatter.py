...
_TIE_TOL = 0.012     # about one marker diameter on the r* axis; named markers closer than this within a tier are dodged
...
def main() -> None:
    ...
    for tk in tiers:
        ...
        named = sorted(np.where((t == tk) & ~on_floor)[0], key=lambda k: rstar[k])
        for i, j in zip(named, named[1:]):
            if rstar[j] - rstar[i] < _TIE_TOL:
                x[i] -= _DODGE / 2
                x[j] += _DODGE / 2
