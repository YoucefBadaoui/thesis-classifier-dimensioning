...
# Hatches on the two tiers whose hues read alike in greyscale.
_TIER_HATCH = {-1: None, 0: None, 1: "///", 2: "xx", 3: None, 4: None, 5: None}
...
def main() -> None:
    ...
    bar_hatch = [_TIER_HATCH[tier_of[n]] for n in names_o]

    fig, ax = plt.subplots(figsize=style.size("full", 3.95))
    x = np.arange(len(names_o))

    # bar height is total per-category support
    ax.bar(x, train_o, color=bar_colors, edgecolor="white", linewidth=0.4, hatch=bar_hatch)
    ax.bar(x, val_o, bottom=train_o, color=bar_colors, alpha=0.72,
           edgecolor="white", linewidth=0.4, hatch=bar_hatch)
    ...
    legend_handles = [
        Patch(facecolor=tier_palette[i], edgecolor="white", hatch=_TIER_HATCH[i],
