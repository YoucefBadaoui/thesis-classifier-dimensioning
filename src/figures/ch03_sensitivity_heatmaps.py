...
def _draw_panel_grid(
    ...
    ``title_format`` is a mathtext string with one ``{cls}`` placeholder. ``vmax_floor`` raises the symmetric colour range to at least ``+/-vmax_floor``. Cell annotations are factored by a shared power of ten when the panel range is below 1e-3; the colourbar factors the same power of ten.
    """
    K = S.shape[0]
    ...
    # Colourbar exponent: the cell factor, so the panel and the bar carry one convention.
    cbar_exp = scale_exp
    ...
    cbar.set_label(
        ...
        + (fr"  ($\times 10^{{{cbar_exp}}}$)" if cbar_exp else ""),
