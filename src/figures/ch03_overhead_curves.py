...
Two figures on one layout: ``03_capacity_overhead_curves`` (OTT/IPTV) and ``03_5g_overhead_curves`` (5G slicing). Each is one row of two panels, (a) capacity overhead with the band spanned by the low-target and high-target families shaded and (b) weighted bandwidth deficit, under a shared legend.
...
# One dash pattern per spillover model, so series identity survives where two curves coincide: in the 5G scenario the uniform and low-target overheads are both identically zero.
_LS_UNIFORM = "solid"
_LS_WORST = (0, (5, 2))
_LS_BEST = (0, (3.5, 1.4, 1, 1.4))

_SERIES_LABELS = ["uniform", "high-target", "low-target"]
_BAND_LABEL = "span of the families"
...
def _add_eps_lines(ax: plt.Axes) -> None:
    ...
    Guides are labelled individually when they are at least one line of text apart on the current y scale; closer guides share one label under the lowest guide, which only a scenario whose curves force a taller y range would reach.
    """
    ...
    if resolved:
        ...
        # one label for all three guides, in the empty band under the lowest guide at the left edge
        levels = ", ".join(str(e) for e in _EPS_LEVELS)
        ax.text(0.015, min(_EPS_LEVELS) / 2, f"$\\varepsilon$ = {levels}%",
...
def _plot_three_lines(
    ...
    """Draw the uniform / high-target / low-target triplet and return handles."""
...
def _build_overhead_figure(
    ...
    ax_oh.set_title("(a) capacity overhead")
    ...
    if np.allclose(oh_u, oh_b):
        ax_oh.text(
            0.035, 0.045, "uniform and low-target\ncoincide at zero",
