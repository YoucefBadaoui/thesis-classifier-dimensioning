...
def _blocking_gradient(V: int, a_hat: np.ndarray, demands: np.ndarray,
    ...
    """Central-difference dB_k/da_hat_j, shape (K, K), step max(|a_hat_j| delta, delta). Meant for strictly positive apparent loads: at a zero load the minus step enters a negative load."""
    K = len(a_hat)
    dB_da = np.zeros((K, K))
    for j in range(K):
...
def capacity_overhead(a_hat: np.ndarray, demands: np.ndarray,
    ...
    Binary search over V. Multirate blocking is not monotone in V in general; the thesis scenarios are non-increasing over the searched range (checked by scan in scripts/checks/verify_core_math.py). Outside that regime use a scan. Relative overhead is Delta_V/V = (V' - V_start) / V_start.
