...
def agg(cm, ct, tiers):
    """Six-tier matrix. Predictions outside the tier map are dropped before each row is normalised, so every row is conditional on a retained prediction; scripts/checks/verify_review_counterexamples.py evaluates the fallback alternative."""
    pos = {t: i for i, t in enumerate(tiers)}
    K = len(tiers)
    o = np.zeros((K, K))
...
def pick_matrix(z, cond, seeds=SEEDS):
    """Return (matrix, seed) for the median-balanced-accuracy seed; the lowest-accuracy seed is never used, the error structure of the chosen one is not inspected."""
    cands = []
    for s in seeds:
        k = f"{cond}_s{s}"
        if k not in z.files:
