...
def main():
    ...
    for tag, res in (("conly", res_c), ("joint", res_j)):
        ...
        # share of replicates whose overhead lands exactly on the nominal capacity
        for li, lab in enumerate(labels):
            if not lab.startswith("dVoverV"):
                continue
            zf = np.mean(res[:, :, li] == 0.0, axis=0)
            save[f"{tag}_{lab}_zerofrac"] = zf
            print("  zero-overhead share " + lab + ": "
                  + "  ".join(f"{c}={f*100:.1f}%" for c, f in zip(conds, zf)))
