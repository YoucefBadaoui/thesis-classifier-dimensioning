...
| `src/iscx/` | Feature list and MLP hyperparameters shared by the ISCX classifier refresh scripts |
...
| `data/archive/` | The leaf-wise LightGBM seed probe (`cesnet_lgbm_default_probe.npz`), a one-off reproduction run that the pipeline does not regenerate |
...
The raw corpora are not redistributed. `data/processed/` and the one probe
under `data/archive/` carry every result archive the thesis quotes, so all
tables and figures can be regenerated and every number in the thesis can be
checked without the corpora. The cleaned ISCX
...
make verify     # verify_core_math, verify_jensen_convexity, verify_rebinning_flip, verify_review_counterexamples, validate_pipeline
```
